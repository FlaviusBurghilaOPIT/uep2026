...
## Acceptance criteria
- [ ] Date label (`dateLine.toUpperCase()`) is clamped with `maxLines: 1` and `overflow: TextOverflow.ellipsis`.
- [ ] Greeting line (`greetingLine`) uses responsive font size sizing with ellipsis overflow on ultra-narrow viewports (<360dp).
- [ ] Layout renders without RenderFlex overflow on 320dp width simulator tests.
