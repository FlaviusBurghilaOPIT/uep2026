...
void main() {
  ...
  testWidgets('OtpScreen renders UI correctly with fallback email text', (WidgetTester tester) async {
    ...
    expect(find.text('Please enter the 6-digit code sent to your email.'), findsOneWidget);
    expect(find.byType(TextFormField), findsOneWidget);
    expect(find.text('Verify and Log In'), findsOneWidget);
  });

  testWidgets('OtpScreen displays email dynamically when email is in state', (WidgetTester tester) async {
    setupScreenSize(tester);
    ...
    expect(find.text('Please enter the 6-digit code sent to test@example.com.'), findsOneWidget);
  });

  testWidgets('OtpScreen validates input length and numeric format (rejects signed/alpha inputs)', (WidgetTester tester) async {
    setupScreenSize(tester);
    SharedPreferences.setMockInitialValues({});
    final prefs = await SharedPreferences.getInstance();
  ...
  testWidgets('OtpScreen submits valid 6-digit code and navigates to main', (WidgetTester tester) async {
    setupScreenSize(tester);
