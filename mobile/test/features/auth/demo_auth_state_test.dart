...
import 'package:remotecare/features/auth/presentation/providers/demo_auth_provider.dart';
...
void main() {
  ...
  group('DemoAuthNotifier', () {
    test(
      'initial state when SharedPreferences is empty defaults to first time',
      () async {
        ...
        await container.read(demoAuthProvider.future);
    ...
    test(
      'completeProfileSetup sets flags and email in state and prefs',
      () async {
        SharedPreferences.setMockInitialValues({});
        final prefs = await SharedPreferences.getInstance();
    ...
    test(
      'completeOtpLogin sets isFirstTime=false and hasActiveSession=true in state and prefs',
      () async {
        SharedPreferences.setMockInitialValues({
